...
func StreamOpenAICompletions(ctx context.Context, model ai.Model, llmContext ai.Context, options OpenAICompletionsOptions) *ai.AssistantMessageEventStream {
	...
	go func() {
		...
		client, requestOptions, err := newClient(model, options.StreamOptions)
		if err != nil {
			pushFinalError(stream, &output, err.Error())
			return
		}
		requestOptions = append(requestOptions, option.WithResponseInto(&rawResponse))
		requestOptions = append(requestOptions, option.WithRequestBody("application/json", params))
		sdkStream := client.Chat.Completions.NewStreaming(ctx, openaisdk.ChatCompletionNewParams{
...
func newClient(model ai.Model, options ai.StreamOptions) (openaisdk.Client, []option.RequestOption, error) {
	config, err := buildOpenAIClientConfig(model, options)
	if err != nil {
...
func buildOpenAIClientConfig(model ai.Model, options ai.StreamOptions) (openAIClientConfig, error) {
	...
	if model.Provider == ai.ProviderCloudflareAIGateway {
		if _, ok := headerValue(model.Headers, options.Headers, "authorization"); !ok {
...
func formatOpenAIError(err error) string {
	for current := err; current != nil; current = errors.Unwrap(current) {
		...
		if !statusField.IsValid() || statusField.Kind() != reflect.Int || statusField.Int() == 0 {
			continue
		}
		message := ""
		...
		return "OpenAI API error (" + strconv.Itoa(int(statusField.Int())) + "): " + message
	}
	return err.Error()
}
