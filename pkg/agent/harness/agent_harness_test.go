...
func TestAgentHarnessProviderHooksPatchOptionsPayloadAndObserveResponse(t *testing.T) {
	...
	harness, err := NewAgentHarness(AgentHarnessOptions{
		Session: session.NewSession(storage),
		Model:   harnessTestModel(),
		StreamOptions: AgentHarnessStreamOptions{Headers: map[string]string{
			"x-base":     "base",
			"x-override": "base",
		}},
		GetAPIKeyAndHeaders: func(ctx context.Context, model ai.Model) (*AgentHarnessAuth, error) {
			return &AgentHarnessAuth{APIKey: "dynamic-key", Headers: map[string]string{
				"x-auth":     "auth",
				"x-override": "auth",
			}}, nil
		},
	...
	if optionsSeen.APIKey != "dynamic-key" {
		t.Fatalf("expected dynamic API key, got %q", optionsSeen.APIKey)
	}
	if requestSessionID != "session-1" {
		t.Fatalf("expected provider request session id, got %q", requestSessionID)
	}
	if optionsSeen.Headers["x-base"] != "base" || optionsSeen.Headers["x-auth"] != "auth" || optionsSeen.Headers["x-override"] != "auth" || optionsSeen.Headers["x-hook"] != "yes" || optionsSeen.Metadata["hook"] != true {
